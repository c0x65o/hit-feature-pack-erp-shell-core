...
export function Reports() {
  ...
  const createReport = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') window.location.href = `/reports/builder?key=${encodeURIComponent(key)}`;
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setCreateBusy(false);
                ...
                {items.map((r) => (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <Button variant="secondary" onClick={() => { window.location.href = `/reports/builder?key=${encodeURIComponent(r.key)}`; }}>
                        Open
                      </Button>
