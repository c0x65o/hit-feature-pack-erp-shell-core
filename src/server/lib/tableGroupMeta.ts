...
export type GroupMetaResult = {
  ...
  groupOrder: string[];
  groups?: Array<{ key: string; total: number; rows: any[] }>;
};
...
export function getEntityByTableId(tableId: string): { entityKey: string; spec: any } | null {
  const entities = getEntities();
...
export function getTableFromSpec(spec: any): any | null {
...
function isVirtualField(fieldSpec: any): boolean {
  if (!fieldSpec || typeof fieldSpec !== 'object') return false;
  if ((fieldSpec as any).virtual === true) return true;
  const storage = (fieldSpec as any).storage;
  if (storage && typeof storage === 'object' && String((storage as any).mode || '').toLowerCase() === 'virtual') {
    return true;
  }
  return false;
}

function getComputeSpec(fieldSpec: any): ComputeSpec | null {
  if (!fieldSpec || typeof fieldSpec !== 'object') return null;
  const compute = (fieldSpec as any).compute;
  if (!compute || typeof compute !== 'object') return null;
  return compute as ComputeSpec;
}

type LddGroupingConfig = {
  assignments: any;
  dimTable: any;
  dimIdCol: AnyColumn;
  dimLabelCol: AnyColumn;
};

function resolveEmployeeLddGrouping(entityKey: string, groupByField: string): LddGroupingConfig | null {
  if (entityKey !== 'hrm.employee') return null;
  if (!schema) return null;
  const assignments = (schema as any).userOrgAssignments;
  const divisions = (schema as any).divisions;
  const departments = (schema as any).departments;
  const locations = (schema as any).locations;
  if (!assignments) return null;
  switch (groupByField) {
    case 'divisionName':
      if (!divisions || !(assignments as any).divisionId || !(divisions as any).name) return null;
      return { assignments, dimTable: divisions, dimIdCol: (assignments as any).divisionId, dimLabelCol: (divisions as any).name };
    case 'departmentName':
      if (!departments || !(assignments as any).departmentId || !(departments as any).name) return null;
      return { assignments, dimTable: departments, dimIdCol: (assignments as any).departmentId, dimLabelCol: (departments as any).name };
    case 'locationName':
      if (!locations || !(assignments as any).locationId || !(locations as any).name) return null;
      return { assignments, dimTable: locations, dimIdCol: (assignments as any).locationId, dimLabelCol: (locations as any).name };
    default:
      return null;
  }
}
...
async function buildEmployeeLddGroupMeta(args: {
  ...
  const userEmailCol = (tableAny as any).userEmail;
  if (!userEmailCol) return null;
  ...
  const baseQuery = db
    ...
    .from(tableAny)
    .leftJoin(assignments, eq(userEmailCol, (assignments as any).userKey))
    .leftJoin(dimTable, eq(dimIdCol, (dimTable as any).id));
  ...
  const entries = (rows as any[]).map((row) => {
    ...
    if (id && row?.label != null) labelMap[id] = String(row.label);
  ...
  if (includeRows) {
    ...
    const rowColumns = getTableColumns(tableAny as any) as any;
    ...
    for (const entry of orderedEntries) {
      ...
      const enriched = (rowsQuery as any[]).map((row) => {
        ...
        if (!out[groupByField] || String(out[groupByField]).trim() === '') {
          ...
          if (label) out[groupByField] = label;
        }
        return out;
      });
      groups.push({
...
async function buildJoinGroupMeta(args: {
  tableId: string;
  table: any;
  joins: Array<{ entity?: string; localField?: string; foreignField?: string }>;
  ...
  const joinSteps: Array<{ table: any; localCol: AnyColumn; foreignCol: AnyColumn }> = [];
  let currentTable = table as any;
  for (const join of joins) {
    ...
    if (!entityKey || !localField || !foreignField) return null;
    const joinSpec = getEntitySpec(entityKey);
    const joinTable = joinSpec ? getTableFromSpec(joinSpec) : null;
    if (!joinTable) return null;
    const localCol = (currentTable as any)[localField];
    const foreignCol = (joinTable as any)[foreignField];
    if (!localCol || !foreignCol) return null;
    joinSteps.push({ table: joinTable, localCol, foreignCol });
    currentTable = joinTable;
  }

  const groupFieldKey = String(groupField || '').trim();
  if (!groupFieldKey) return null;
  const groupCol = (currentTable as any)[groupFieldKey];
  if (!groupCol) return null;

  const labelFieldKey = String(labelField || '').trim();
  const labelCol = labelFieldKey ? (currentTable as any)[labelFieldKey] : null;
  const orderFieldKey = String(orderField || '').trim();
  const orderCol = orderFieldKey ? (currentTable as any)[orderFieldKey] : null;

  const db = getDb();
  let baseQuery = db
    .select({
      groupValue: groupCol,
      label: labelCol ?? sql<string | null>`null`,
      count: sql<number>`count(*)`,
      sortOrder: orderCol ?? sql<number | null>`null`,
    })
    .from(table);
  ...
  const groupByCols: AnyColumn[] = [groupCol];
  if (labelCol) groupByCols.push(labelCol as AnyColumn);
  if (orderCol) groupByCols.push(orderCol as AnyColumn);
  const rows = whereClause
  ...
  const entries = (rows as any[]).map((row) => {
    ...
    if (row?.sortOrder != null) {
      ...
      if (Number.isFinite(n)) orderMap[id] = n;
    }
    groupCounts[key] = (groupCounts[key] ?? 0) + count;
...
export async function buildTableGroupMeta(args: {
  ...
  includeRows?: boolean;
  groupPageSize?: number;
  view?: { sorting?: Array<{ id?: string; desc?: boolean }> } | null;
  currentUserId?: string | null;
  entity?: { entityKey: string; spec: any } | null;
  table?: any | null;
}): Promise<GroupMetaResult | GroupMetaError> {
  const tableId = String(args.tableId || '').trim();
  if (!tableId) return { error: 'Missing tableId', status: 400 };

  const groupByField = typeof args.groupBy?.field === 'string' ? args.groupBy.field.trim() : '';
  if (!groupByField) return { error: 'Missing groupBy.field', status: 400 };

  const entity = args.entity ?? getEntityByTableId(tableId);
  if (!entity) return { error: `Unknown tableId=${tableId}`, status: 404 };

  const uiSpec = entity.spec;
  const table = args.table ?? getTableFromSpec(uiSpec);
  if (!table) return { error: 'Missing storage.drizzleTable', status: 500 };

  const filters = Array.isArray(args.filters) ? args.filters : [];
  const filterMode: 'all' | 'any' = args.filterMode === 'any' ? 'any' : 'all';
  ...
  let groupCol = (table as any)[effectiveField];
  ...
  if (!groupCol && isVirtual && computeSpec?.kind === 'labelFrom') {
    ...
    const sourceCol = sourceField ? (table as any)[sourceField] : null;
    if (sourceCol) {
  ...
  if (!groupCol && isVirtual && computeSpec?.kind === 'concat') {
    ...
    const cols = fieldKeys
      ...
      .filter(Boolean)
      .map((k) => (table as any)[k])
      .filter(Boolean) as AnyColumn[];
    if (cols.length === 0) {
      return { error: `Unknown groupBy field: ${groupByField}`, status: 400 };
    }

    const concatExpr = sql<string>`concat_ws(${sep}, ${sql.join(cols, sql`, `)})`;
    const db = getDb();
    const rows = whereClause
      ? await db.select({ groupValue: concatExpr, count: sql<number>`count(*)` }).from(table).where(whereClause).groupBy(concatExpr)
      : await db.select({ groupValue: concatExpr, count: sql<number>`count(*)` }).from(table).groupBy(concatExpr);
    ...
    if (includeRows) {
      const tableAny = table as any;
      const sortId = Array.isArray((args.view as any)?.sorting) && (args.view as any).sorting.length > 0
        ? String((args.view as any).sorting[0]?.id || '')
        : '';
      const sortDesc = Array.isArray((args.view as any)?.sorting) && (args.view as any).sorting.length > 0
        ? Boolean((args.view as any).sorting[0]?.desc)
        : false;
      const rowOrderCol = (sortId && (tableAny as any)[sortId]) ? (tableAny as any)[sortId] : (tableAny as any).id;
      const rowOrder = sortDesc ? desc(rowOrderCol) : asc(rowOrderCol);
  ...
  if (!groupCol && isVirtual && computeSpec?.kind === 'join') {
    ...
    if (joinResult) return joinResult as GroupMetaResult;
  }
  ...
  if (!groupCol) {
    ...
    if (lddGrouping) {
      ...
      if (lddResult) return lddResult as GroupMetaResult;
    }
    return { error: `Unknown groupBy field: ${groupByField}`, status: 400 };
  }
  ...
  const optionSource = String((labelFieldSpec as any)?.optionSource || '').trim();
  const referenceEntity = String((labelFieldSpec as any)?.reference?.entityType || '').trim();
