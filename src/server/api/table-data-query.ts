...
function resolveListEndpoint(entityKey: string, spec: any): string | null {
  const api = spec && typeof spec === 'object' ? (spec as any).api : null;
...
export async function POST(request: NextRequest) {
  ...
  let sortOrder: 'asc' | 'desc' = body.sortOrder === 'desc' ? 'desc' : 'asc';
  ...
  if (viewId) {
    ...
    if (!sortBy && Array.isArray((view as any)?.sorting) && (view as any).sorting.length > 0) {
      const first = (view as any).sorting[0];
  ...
  if (includeGroups) {
    ...
    const groupMetaBody: Record<string, unknown> = viewId
      ? { viewId }
      : {
          tableId,
          groupBy: {
    ...
    if (search) groupMetaBody.search = search;
    ...
    const groupsRaw = Array.isArray((groupJson as any)?.groups) ? (groupJson as any).groups : [];
    const items = groupsRaw.flatMap((g: any) => (Array.isArray(g?.rows) ? g.rows : []));
  ...
  if (!listEndpoint) return jsonError('Missing api.baseUrl or api.resource', 400);
  ...
  if (filters.length > 0) qp.set('filterMode', filterMode);
  ...
  const items = Array.isArray((json as any)?.items)
    ...
    : Array.isArray((json as any)?.data?.items)
      ...
      : Array.isArray(json)
        ? json
        : [];
  const pagination = (json as any)?.pagination || (json as any)?.data?.pagination || {
    page,
    pageSize,
